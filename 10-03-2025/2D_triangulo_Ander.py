...
ex = 2.0  # Escala no eixo X
ey = 2.0  # Escala no eixo Y
ez = 2.0  # Escala no eixo Z (não afeta 2D)
zoom_factor = -5  # Valor inicial do zoom corrigido

def setup_projection():
    glMatrixMode(GL_PROJECTION)
    glLoadIdentity()
    gluPerspective(45, display[0] / display[1], 0.1, 50.0)
    glTranslatef(0.0, 0.0, zoom_factor)  # Aplica o zoom no eixo Z
    glMatrixMode(GL_MODELVIEW)
    
    
# Função de inicialização do OpenGL
def init():
    glClearColor(1.0, 1.0, 1.0, 1.0)  # Define a cor de fundo azul (R=0, G=0, B=1, A=1)
    ...
    zoom_factor = -5  # Valor inicial do zoom (mais negativo = mais distante)
...
def draw():
    ...
    glTranslatef(x, y, zoom_factor)  # Alterado para usar zoom_factor  
    ...
    glColor3f(0.0, 0.0, 0.0)  # Define a cor do triângulo como amarelo (R=1, G=1, B=0)
    glVertex3f(1, 2, 1)  # Define o vértice superior
    glVertex3f(0, 0, 1)  # Define o vértice inferior esquerdo
    glVertex3f(2, 0, 1)  # Define o vértice inferior direito
...
def main():
    ...
    global x, y, r, ex, ey, ez, zoom_factor  # Declara as variáveis globais para uso dentro da função
    ...
    while running:
        for event in pygame.event.get():  # Captura eventos do sistema (teclado, mouse, etc.)
            ...
            if event.type == KEYDOWN:  # Se uma tecla for pressionada
                if event.key ==K_z:  # Tecla "Z" para aproximar
                        
                    zoom_factor += 1
                if event.key == K_x:  # Tecla "X" para afastar   
                     zoom_factor -= 1 
